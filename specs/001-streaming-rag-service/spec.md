...
- **FR-007**: System MUST support provider adapters and runtime provider selection (e.g., local provider runtime, OpenAI remote) via a **per-deployment ordered provider list** in server configuration. **Decision**: MVP uses per-deployment config only (single ordered list, e.g., `[Ollama, OpenAI]`); per-org provider priority is deferred. **Acceptance**: tests demonstrate provider selection follows configured order, local-first behavior, and fallback to the next provider on failure.
