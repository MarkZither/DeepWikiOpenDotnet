...
/// <summary>
/// SQL Server EF Core implementation of <see cref="IWikiRepository"/>.
/// Uses <see cref="SqlServerVectorDbContext"/> with eager loading for related entities.
/// </summary>
public class SqlServerWikiRepository : IWikiRepository
{
    private readonly SqlServerVectorDbContext _context;

    public SqlServerWikiRepository(SqlServerVectorDbContext context)
    {
    ...
    {
        // Cascade delete is configured on WikiPages.WikiId — deleting the wiki removes pages;
        // SourceRelations also cascade. TargetRelations require manual cleanup first
        // (SQL Server multiple-cascade-path limitation; handled via NoAction FK).
        await _context.WikiPageRelations
            .Where(r => r.TargetPage.WikiId == wikiId)
    ...
    {
        // SQL Server NoAction FK on WikiPageRelations.TargetPageId requires manual cleanup
        // of inbound relations before the page can be deleted.
        await _context.WikiPageRelations
            .Where(r => r.TargetPageId == pageId)
            .ExecuteDeleteAsync(cancellationToken);
